...
class FullyConnected(object):
    ...
    def output_mix(self,input):
        lin_output = T.dot(T.concatenate([input[0],input[1]],axis=1), self.W) + self.b
        return (
        lin_output if self.activation is None
             else self.activation(lin_output)
        )

    def output_mix2(self,input):
        lin_output = T.dot(input[0] +input[1], self.W) + self.b
        return (
        lin_output if self.activation is None
             else self.activation(lin_output)
        )
