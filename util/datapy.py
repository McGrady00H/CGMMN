...
def labelToMat(y):
    label = np.unique(y)
    newy = np.zeros((len(y), len(label))).astype(np.float32)
    for i in range(len(y)):
        newy[i, y[i]] = 1
    return newy

def labelToMat_with_noise(y):
    label = np.unique(y)
    newy = (np.random.random((len(y), len(label)))/100).astype(np.float32)
    for i in range(len(y)):
        newy[i, y[i]] = newy[i,y[i]]+ 1
    return newy
...
def load_data_cifar(dataset, have_matrix = False):
    ...
    train_x = np.cast['float32'](train_x)
    train_x = train_x / 255
    test_x = np.cast['float32'](test_x)
    test_x = test_x / 255
    ...
    def shared_dataset(data_xy, borrow=True):
        ...
        # ``shared_y`` we will have to cast it to int. This little hack
        # lets ous get around this issue
        return shared_x, T.cast(shared_y, 'int32'), T.cast(shared_y_matrix, 'float32')
...
def load_data_cifar_p(dataset, have_matrix = False):
    ''' 
    Loads the dataset
    '''
    data = np.load('./data/cifar-10-batches-py/'+dataset+'.npz')
    #train_x, train_y = data['data']
    train_xx = data['train_xx_0'] 
    train_x = data['train_x_0']
    train_y = data['train_y_0']
    train_label = data['train_label']
    train_x = np.cast['float32'](train_x)
    train_x = train_x / 255
    train_xx = np.cast['float32'](train_xx)
    train_xx = train_xx / 255
    train_y = np.cast['float32'](train_y)
    train_y = train_y / 255
    ...
    def shared_dataset(data_xy, borrow=True):
        ...
        variable) would lead to a large decrease in performance.
        """
        data_xx, data_x, data_y,data_label = data_xy

        label_matrix = labelToMat(np.asarray(data_label))

        shared_x = theano.shared(np.asarray(data_x,
                                               dtype=theano.config.floatX),
                                 borrow=borrow)
        shared_xx = theano.shared(np.asarray(data_xx,
                                               dtype=theano.config.floatX),
                                 borrow=borrow)
        shared_y = theano.shared(np.asarray(data_y,
                                               dtype=theano.config.floatX),
                                 borrow=borrow)
        shared_label = theano.shared(np.asarray(label_matrix,
                                               dtype=theano.config.floatX),
                                 borrow=borrow)
        ...
        # ``shared_y`` we will have to cast it to int. This little hack
        # lets ous get around this issue
        return shared_xx, shared_x, shared_y, T.cast(shared_label, 'float32')

    train_set_xx, train_set_x, train_set_y, train_set_label = shared_dataset([train_xx, train_x, train_y, train_label])

    rval = [(train_set_xx, train_set_x, train_set_y, train_set_label)]
    return rval

def load_data_cifar_p_16(dataset, have_matrix = False):
    ''' 
    Loads the dataset
    '''
    data = np.load('./data/cifar-10-batches-py/'+dataset+'.npz')
    #train_x, train_y = data['data']
    train_xx = data['train_xx_1'] 
    train_x = data['train_x_1']
    train_y = data['train_y_1']
    train_label = data['train_label']
    train_x = np.cast['float32'](train_x)
    train_x = train_x / 255
    train_xx = np.cast['float32'](train_xx)
...
def load_data_cifar_p_8(dataset, have_matrix = False):
    ''' 
    Loads the dataset
    '''
    data = np.load('./data/cifar-10-batches-py/'+dataset+'.npz')
...
def load_data_cifar_p_8_aug_16(dataset, have_matrix = False):
    ''' 
    Loads the dataset
    '''
    data = np.load('./data/cifar-10-batches-py/'+dataset+'.npz')
    ...
    train_y = data['train_y_1']
    train_label = data['train_label']
    data = np.load('./data/cifar-10-batches-py/'+dataset+'_aug_16.npz')
    train_x = data['train_x_aug']
    train_x = np.cast['float32'](train_x)
    train_x = train_x / 255
    train_xx = np.cast['float32'](train_xx)
    train_xx = train_xx / 255
...
def load_data_cifar_p_8_aug(dataset, have_matrix = False):
    ''' 
    Loads the dataset
    '''
    data = np.load('./data/cifar-10-batches-py/'+dataset+'.npz')
    ...
    train_y = data['train_y_1']
    train_label = data['train_label']
    data = np.load('./data/cifar-10-batches-py/'+dataset+'_aug.npz')
    train_x = data['train_x_aug']
    train_x = np.cast['float32'](train_x)
    train_x = train_x / 255
    train_xx = np.cast['float32'](train_xx)
...
def load_data_gpu_60000(dataset, have_matrix = False):
    ...
    def shared_dataset(data_xy, borrow=True):
        ...
        # lets ous get around this issue
        #return shared_x, T.cast(shared_y, 'int32'), T.cast(shared_y_matrix, 'int32')
        return shared_x, shared_y, shared_y_matrix
...
def load_data_gpu_60000_with_noise(dataset, have_matrix = False):
    ''' 
    Loads the dataset
    ...
    def shared_dataset(data_xy, borrow=True):
        ...
        y_matrix = labelToMat_with_noise(np.asarray(data_y))
        shared_y_matrix = theano.shared(y_matrix)
